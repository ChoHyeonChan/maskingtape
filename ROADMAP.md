...
| 이름 (규칙) | 0.973 | 0.691 | 0.808 |
| **전체** | **0.992** | **0.887** | **0.937** |

이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. #150(NameDetector 존칭 삼킴 버그 수정, #147)과 #160(단어 중간 성씨 오탐 수정, #158)으로 규칙판 이름 F1이 **0.676 → 0.827**로 올랐습니다. #158은 부분만 해결됐습니다 — 남은 오탐 패턴은 bench/README.md의 confidence 절 참고. 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 하며, 상세는 [bench/README.md](bench/README.md) 참고. 주민등록번호는 #159에서 체크섬 없는(2020-10 이후 발급분) 케이스도 추가했는데, precision/recall엔 영향이 없지만(core가 체크섬과 무관하게 탐지는 하므로) confidence가 0.85로 낮아져 confidence 임계값을 0.9 이상으로 올리면 정답 RRN의 19%가 걸러진다는 걸 확인했습니다. 계좌번호(#180)는 그보다 더 심각합니다 — core `AccountDetector`가 체크섬이 없어 confidence를 **항상 정확히 0.6**으로 고정해서, **confidence 임계값을 0.7 이상으로만 올려도 계좌번호가 100% 걸러집니다**(확률적인 RRN과 달리 확정적). 주소(#195)는 동/번지 없는 부분 주소(명함·채용공고 등)를 처음 추가하자 recall이 1.000→0.968로 떨어졌는데, 확신도 문제가 아니라 **시/도명 뒤에 조사가 공백 없이 바로 붙으면 core가 아예 못 잡는 버그**를 새로 발견했습니다(#196, core 대응 대기) — 자세한 내용은 bench/README.md의 confidence 절 참고.

...
합성 평가 데이터셋 500건 기준 (`python -m bench.evaluators.evaluate`, **규칙 전용 모드**, 측정 기준 `dce78a5`):
...
| 이름 (규칙) | 0.924 | 0.531 | 0.675 |
| **전체** | **0.981** | **0.825** | **0.896** |

> 여권번호(`passport`, #139)는 PR #140이 아직 리뷰 대기 중이라 이 표에는 반영 전입니다.

이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. **로컬 LLM 하이브리드**로 켜면 이름 F1이 **0.675 → 0.951**(precision 0.924→0.987, recall 0.531→0.918)로 오릅니다 — "왜 하이브리드인가"의 근거이자 발표 핵심입니다. #128(LLM 반환값의 존칭 '님'/'씨' 스팬 정합 수정) 이후 재측정한 수치입니다(#141) — 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 합니다.

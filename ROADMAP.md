...
| 이름 (규칙) | 0.977 | 0.681 | 0.803 |
| **전체** | **0.993** | **0.885** | **0.936** |

이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. #150(NameDetector 존칭 삼킴 버그 수정, #147)과 #160(단어 중간 성씨 오탐 수정, #158)으로 규칙판 이름 F1이 **0.676 → 0.803**으로 올랐습니다. #158은 부분만 해결됐습니다 — 남은 오탐 패턴은 bench/README.md의 confidence 절 참고. 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 하며, 상세는 [bench/README.md](bench/README.md) 참고. 주민등록번호는 #159에서 체크섬 없는(2020-10 이후 발급분) 케이스도 추가했는데, precision/recall엔 영향이 없지만(core가 체크섬과 무관하게 탐지는 하므로) confidence가 0.85로 낮아져 **confidence 임계값을 0.9 이상으로 올리면 정답 RRN의 19%가 걸러진다**는 걸 처음으로 확인했습니다 — 자세한 내용은 bench/README.md의 confidence 절 참고.

...
이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. **로컬 LLM 하이브리드**로 켜면 이름 F1이 **0.676 → 0.950**(precision 0.928→0.971, recall 0.532→0.931)으로 오릅니다 — "왜 하이브리드인가"의 근거이자 발표 핵심입니다. #128(LLM 반환값의 존칭 '님'/'씨' 스팬 정합 수정) 이후, passport(#139) 반영된 최신 데이터셋 기준으로 재측정한 수치입니다(#141) — 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 합니다.

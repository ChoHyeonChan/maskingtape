...
합성 평가 데이터셋 500건 기준 (`python -m bench.evaluators.evaluate`, **규칙 전용 모드**, 측정 기준 `090fb88` — #139 반영):
...
| 이름 (규칙) | 0.928 | 0.532 | 0.676 |
| **전체** | **0.982** | **0.829** | **0.899** |

이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. **로컬 LLM 하이브리드**로 켜면 이름 F1이 대략 **0.676 → 0.93 안팎**으로 오릅니다(재현율도 크게 상승) — "왜 하이브리드인가"의 근거이자 발표 핵심입니다. 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 하며, #128(LLM 이름 존칭 정합 수정)이 머지된 뒤로 아직 재측정 전입니다.

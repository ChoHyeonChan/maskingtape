...
합성 평가 데이터셋 500건 기준 (`python -m bench.evaluators.evaluate`, **규칙 전용 모드**, 측정 기준 `<COMMIT_HASH>` — #150 반영):
...
| 이름 (규칙) | 0.982 | 0.699 | 0.817 |
| **전체** | **0.995** | **0.890** | **0.940** |

이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. #150(NameDetector 존칭 '님'/'씨' 삼킴 버그 수정, #147)으로 규칙판 이름 F1이 **0.676 → 0.817**로 크게 올랐습니다(FP·FN 이중 집계가 해소됨). **로컬 LLM 하이브리드**로 켜면 F1이 **0.817 → 0.950**(precision 0.982→0.971, recall 0.699→0.931)으로 더 오릅니다 — 다만 이 하이브리드 수치 자체는 #150 이후로 변화가 없습니다(하이브리드는 규칙판을 `min_confidence=0.75` 안전망으로만 써서, confidence 0.5짜리였던 #150의 버그 케이스는 애초에 하이브리드 결과에 안 들어가고 있었습니다). 즉 #150은 **규칙 전용 모드 사용자에게만** 의미 있는 개선입니다. 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 합니다.

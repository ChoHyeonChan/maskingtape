...
- [x] REST 전환 — #188. **CLI 우선 + `apps/api` 폴백**으로 넣었다(갈아끼우지 않았다): `pseudonym`과 이름 정밀 탐지를 API가 지원하지 않아 CLI를 지우면 기능이 후퇴한다. CLI 없는 PC에서는 REST로 넘어가 그대로 동작한다. 선결 조건이던 `DetectionKind`의 `passport` 누락은 #157에서 해결됐고, `pseudonym`은 #309에서 API가 지원하게 되어 REST 경로로도 열렸다(이름 정밀 탐지는 여전히 CLI 전용)
- [x] 신규 kind 라벨 추종 — #81(card), #135(biz_reg·passport), #186(account), #284(birth_date). 모르는 kind는 `기타(kind)`로 안전하게 표시(#220/#232)
...
합성 평가 데이터셋 500건 기준 (`python -m bench.evaluators.evaluate`, **규칙 전용 모드**, 측정 기준 #394/#399 규칙판 정비 병합 시점 main — #315(운전면허 신규 kind)·이름 규칙판 정비 전부 반영):
...
| 이름 (규칙) | 0.954 | 0.869 | 0.910 |
| **전체** | **0.983** | **0.950** | **0.966** |

**생년월일(`birth_date`)은 #266/#271에서 core가 추가한 10번째 kind입니다** — bench가 처음으로 데이터를 붙였습니다(#275). "생년월일/생일/출생일" 앵커 뒤의 날짜만 잡는 문맥 앵커 방식(account와 동일한 하드 게이트 패턴)이라, confidence가 **항상 정확히 0.9로 고정**됩니다 — 임계값 0.91 이상이면 100% 소실됩니다(확정적, account와 같은 급의 위험). **운전면허번호(`driver_license`)는 #267/#305에서 core가 추가한 11번째 kind입니다** — bench가 처음으로 데이터를 붙였습니다(#315). 체크섬이 비공개라 형식+지역코드(11~26·28)만으로 판단해 confidence가 **항상 정확히 0.85로 고정**되고, birth_date/account와 달리 **문맥 앵커 게이트조차 없습니다** — 지역코드만 우연히 맞으면 어떤 12자리 숫자든 무조건 오탐됩니다. 재측정 중 계좌번호 생성기 자신의 출력이 이 경계와 core `PhoneDetector`의 050 정규식 양쪽에 우연히 걸리는 걸 실측으로 찾아, 생성기에 회귀 가드를 추가해 고쳤습니다(상세는 [bench/README.md](bench/README.md)). 이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. #150(NameDetector 존칭 삼킴 버그 수정, #147)과 #160(단어 중간 성씨 오탐 수정, #158)으로 규칙판 이름 F1이 **0.676 → 0.827**까지 올랐다가, #213(직함이 이름 뒤)·#239(직함이 이름 앞)로 직함-only 이름 지원이 넓어지면서 정밀도가 떨어졌습니다. 그 오탐 방지 가드가 조사에 뚫리던 버그(#247)를 core #252가 고쳤지만 **완전히는 아닙니다** — 하드코딩 5개 부서어(구매/정기/홍보/안전/노무)만 막는 방식이라, 그 밖의 흔한 업무어("차량"/"허가"/"성과"/"안내")는 여전히 뚫렸습니다. #255에서 이 잔여 위험을 negative 데이터로 처음 정량화했고(유출이 아니라 과다 마스킹 문제 — negative 문서엔 가릴 개인정보가 없음), #394/#399 규칙판 정비가 정지어를 일반명사 목록으로 넓히고 **단어 경계**로 걸러 negative 오탐 23건을 0건으로 없앴습니다. 같은 정비에서 실무 직함·서식 역할어 어휘 확장과 역할어 뒤 조사 처리로 규칙판 이름 F1이 **0.752 → 0.910**(recall 0.668 → 0.869)이 됐고, "이용"·"이유"를 startswith로 걸러 실명(이용재·이유진)이 통째로 새던 유출 버그도 같이 잡았습니다. #158은 2026-08-10 재오픈됨(성씨가 단어 시작인 역할어에서 여전히 재현) — 남은 오탐 패턴은 bench/README.md의 confidence 절 참고. 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 하며, 상세는 [bench/README.md](bench/README.md) 참고. 주민등록번호는 #159에서 체크섬 없는(2020-10 이후 발급분) 케이스, #209에서 점(.) 구분자 표기도 추가했는데, precision/recall엔 영향이 없지만(core가 형식만으로 탐지는 하므로) 체크섬 없는 케이스는 confidence가 0.85로 낮아져 confidence 임계값을 0.9 이상으로 올리면 정답 RRN의 13.2%가 걸러진다는 걸 확인했습니다(#339/#340 재측정 기준). 전화번호는 #211에서 050X 평생번호·안심번호도 새로 잡게 됐습니다. 계좌번호(#180)는 그보다 더 심각합니다 — core `AccountDetector`가 체크섬이 없어 confidence를 **항상 정확히 0.6**으로 고정해서, **confidence 임계값을 0.7 이상으로만 올려도 계좌번호가 100% 걸러집니다**(확률적인 RRN과 달리 확정적). 주소는 시/도명 뒤 **조사**(#195/#196) 미탐과 **계사 어미**(#248) 미탐을 둘 다 발견했는데, core #252가 모두 고쳐 recall이 1.000으로 완전히 복구됐습니다(직접 재확인함). confidence 0.5~0.65 구간은 여전히 남아있어 임계값 0.7 이상에서 새기 쉽습니다 — 자세한 내용은 bench/README.md의 confidence 절 참고.

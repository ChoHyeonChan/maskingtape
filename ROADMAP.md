...
| 이름 (규칙) | 0.984 | 0.661 | 0.791 |
| **전체** | **0.996** | **0.876** | **0.932** |

이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. #150(NameDetector 존칭 삼킴 버그 수정, #147)과 #160(단어 중간 성씨 오탐 수정, #158)으로 규칙판 이름 F1이 **0.676 → 0.827**까지 올랐다가, #213(직함-only 이름 지원)으로 그 케이스 중 2음절 이름을 설계상 여전히 놓치는 게 반영돼 0.791로 소폭 낮아졌습니다(core 오탐 방지 가드 — 버그 아님, 부서어가 이름으로 오탐되는 걸 막는 트레이드오프). #158은 2026-08-10 재오픈됨(성씨가 단어 시작인 역할어에서 여전히 재현) — 남은 오탐 패턴은 bench/README.md의 confidence 절 참고. 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 하며, 상세는 [bench/README.md](bench/README.md) 참고. 주민등록번호는 #159에서 체크섬 없는(2020-10 이후 발급분) 케이스, #209에서 점(.) 구분자 표기도 추가했는데, precision/recall엔 영향이 없지만(core가 형식만으로 탐지는 하므로) 체크섬 없는 케이스는 confidence가 0.85로 낮아져 confidence 임계값을 0.9 이상으로 올리면 정답 RRN의 12.7%가 걸러진다는 걸 확인했습니다. 전화번호는 #211에서 050X 평생번호·안심번호도 새로 잡게 됐습니다. 계좌번호(#180)는 그보다 더 심각합니다 — core `AccountDetector`가 체크섬이 없어 confidence를 **항상 정확히 0.6**으로 고정해서, **confidence 임계값을 0.7 이상으로만 올려도 계좌번호가 100% 걸러집니다**(확률적인 RRN과 달리 확정적). 주소(#195)는 동/번지 없는 부분 주소(명함·채용공고 등)를 추가하며 시/도명 뒤 조사 미탐 버그를 발견했었지만(#196), 머지되어 recall 1.000으로 복구됐습니다 — 다만 confidence 0.5~0.65 구간은 남아있어 임계값 0.7 이상에서 여전히 새기 쉽습니다 — 자세한 내용은 bench/README.md의 confidence 절 참고.

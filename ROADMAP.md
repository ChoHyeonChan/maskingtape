...
- [x] REST 전환 — #188. **CLI 우선 + `apps/api` 폴백**으로 넣었다(갈아끼우지 않았다): `pseudonym`과 이름 정밀 탐지를 API가 지원하지 않아 CLI를 지우면 기능이 후퇴한다. CLI 없는 PC에서는 REST로 넘어가 그대로 동작한다. 선결 조건이던 `DetectionKind`의 `passport` 누락은 #157에서 해결됐고, `pseudonym`은 #296에서 API가 지원하면 REST 경로로도 열린다

...
합성 평가 데이터셋 500건 기준 (`python -m bench.evaluators.evaluate`, **규칙 전용 모드**, 측정 기준 `4a13561` — #255(부서어 오탐 잔여 위험 정량화) 전부 반영, 해시는 이 PR 머지 후 갱신):
...
| 이름 (규칙) | 0.832 | 0.679 | 0.748 |
| **전체** | **0.943** | **0.876** | **0.908** |

이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. #150(NameDetector 존칭 삼킴 버그 수정, #147)과 #160(단어 중간 성씨 오탐 수정, #158)으로 규칙판 이름 F1이 **0.676 → 0.827**까지 올랐다가, #213(직함이 이름 뒤)·#239(직함이 이름 앞)로 직함-only 이름 지원이 넓어지면서 정밀도가 떨어졌습니다. 그 오탐 방지 가드가 조사에 뚫리던 버그(#247)를 core #252가 고쳤지만 **완전히는 아닙니다** — 하드코딩 5개 부서어(구매/정기/홍보/안전/노무)만 막는 방식이라, 그 밖의 흔한 업무어("차량"/"허가"/"성과"/"안내")는 여전히 뚫립니다. #255에서 이 잔여 위험을 negative 데이터로 처음 정량화했더니 **negative 문서의 26%(33/125건)에서 재현**됐고, precision이 오히려 더 떨어졌습니다(0.832) — 다만 이건 유출이 아니라 과다 마스킹 문제입니다(negative 문서엔 애초에 가릴 개인정보가 없음). #158은 2026-08-10 재오픈됨(성씨가 단어 시작인 역할어에서 여전히 재현) — 남은 오탐 패턴은 bench/README.md의 confidence 절 참고. 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 하며, 상세는 [bench/README.md](bench/README.md) 참고. 주민등록번호는 #159에서 체크섬 없는(2020-10 이후 발급분) 케이스, #209에서 점(.) 구분자 표기도 추가했는데, precision/recall엔 영향이 없지만(core가 형식만으로 탐지는 하므로) 체크섬 없는 케이스는 confidence가 0.85로 낮아져 confidence 임계값을 0.9 이상으로 올리면 정답 RRN의 15.6%가 걸러진다는 걸 확인했습니다. 전화번호는 #211에서 050X 평생번호·안심번호도 새로 잡게 됐습니다. 계좌번호(#180)는 그보다 더 심각합니다 — core `AccountDetector`가 체크섬이 없어 confidence를 **항상 정확히 0.6**으로 고정해서, **confidence 임계값을 0.7 이상으로만 올려도 계좌번호가 100% 걸러집니다**(확률적인 RRN과 달리 확정적). 주소는 시/도명 뒤 **조사**(#195/#196) 미탐과 **계사 어미**(#248) 미탐을 둘 다 발견했는데, core #252가 모두 고쳐 recall이 1.000으로 완전히 복구됐습니다(직접 재확인함). confidence 0.5~0.65 구간은 여전히 남아있어 임계값 0.7 이상에서 새기 쉽습니다 — 자세한 내용은 bench/README.md의 confidence 절 참고.

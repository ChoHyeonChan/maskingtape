...
합성 평가 데이터셋 500건 기준 (`python -m bench.evaluators.evaluate`, **규칙 전용 모드**, 측정 기준 `9cacf69`):
...
| 이름 (규칙) | 0.881 | 0.509 | 0.645 |
| **전체** | **0.968** | **0.811** | **0.883** |

이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. **로컬 LLM 하이브리드**로 켜면 이름 F1이 대략 **0.645 → 0.93 안팎**으로 오릅니다(재현율도 크게 상승) — "왜 하이브리드인가"의 근거이자 발표 핵심입니다. 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 하며, 이 표에는 아직 #118/#123(리뷰 대기 중인 PR)의 시/도 없는 주소·사업자등록번호 데이터가 반영되지 않았습니다 — 머지되면 다시 갱신 필요.

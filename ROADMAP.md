...
- [x] 새 탐지기 나오면 해당 종류 벤치 데이터 추가 — card(#73/#88), biz_reg(#123), passport(#139) 완료. 상시 진행 항목이라 계속 열어둠

## 현재 정확도

합성 평가 데이터셋 500건 기준 (`python -m bench.evaluators.evaluate`, **규칙 전용 모드**, 측정 기준 `<COMMIT_HASH>` — #148/#145/#150 전부 반영):
...
| 이름 (규칙) | 0.964 | 0.675 | 0.794 |
| **전체** | **0.990** | **0.884** | **0.934** |

이름은 문맥 판단이 필요해 규칙만으로는 한계가 있습니다. #150(NameDetector 존칭 '님'/'씨' 삼킴 버그 수정, #147)으로 규칙판 이름 F1이 **0.676 → 0.794**로 크게 올랐습니다(FP·FN 이중 집계가 해소됨). **로컬 LLM 하이브리드**로 켜면 F1이 **0.794 → 0.935**(precision 0.964→0.967, recall 0.675→0.906)로 더 오릅니다 — 다만 이 하이브리드 수치 자체는 #150 이후로 거의 변화가 없습니다(하이브리드는 규칙판을 `min_confidence=0.75` 안전망으로만 써서, confidence 0.5짜리였던 #150의 버그 케이스는 애초에 하이브리드 결과에 거의 안 들어가고 있었습니다). 즉 #150은 **규칙 전용 모드 사용자에게 특히** 의미 있는 개선입니다. 정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정해야 합니다.

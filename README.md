...
**측정 기준: `56bb22c` · 규칙 전용 모드(LLM 미사용)**
...
| 이름 (규칙 전용) | 0.916 | 0.507 | 0.653 |
| **전체** | **0.980** | **0.820** | **0.893** |

번호·주소·카드·사업자등록번호는 형태와 체크섬으로 완전히 잡힌다(유선전화·plus 이메일·서브도메인·
여러 문장으로 구성된 복합 문서까지 섞어도 흔들리지 않음을 확인했다). 이름만 남은 과제다:

- **이름 (recall 0.507)** — 한국어 이름은 형태만으로 구분되지 않아 규칙만으로는 문맥 없는 이름을 놓친다. 이것이 **로컬 LLM 하이브리드**(`--llm`)가 필요한 이유이고, 그 효과는 [#46](https://github.com/ChoHyeonChan/maskingtape/issues/46)에서 같은 데이터셋으로 비교 측정한다 — 하이브리드로 켜면 F1이 **0.653 → 0.933 안팎**으로 오른다(정확한 하이브리드 수치는 데이터셋이 바뀔 때마다 로컬 Ollama로 재측정 필요 — 상세는 [bench/](bench/) 참고).

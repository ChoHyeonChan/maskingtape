...
| 놓치는 입력 | 잡히는 입력 | 종류 |
|---|---|---|
| `전라남도 해남군 해남읍 중앙1로 330` | `서울특별시 강남구 테헤란로 12` | 주소 — 읍·면 뒤에 도로명이 오는 주소. `…해남읍`까지만 잡고 `중앙1로 330`이 샌다([#423](https://github.com/ChoHyeonChan/maskingtape/issues/423)) |
| `서울특별시 강남구 대동로 12` | `서울특별시 강남구 테헤란로 12` | 주소 — 도로명 안에 동·리가 든 이름. `…대동`까지만 잡고 `로 12`가 샌다([#423](https://github.com/ChoHyeonChan/maskingtape/issues/423)) |
| `부산광역시 해운대구 센텀2로 25` | `서울특별시 종로구 종로1길 50` | 주소 — 도로명 중간에 숫자. `…해운대구`까지만 잡고 `센텀2로 25`가 샌다([#423](https://github.com/ChoHyeonChan/maskingtape/issues/423)) |
...
| `생년월일 95.03.22` | `생년월일 1995년 3월 22일` | 생년월일 — 2자리 연도 |

**주소 세 줄(#423)은 구간이 중간에서 끊겨 건물번호가 새는 부분 유출이라 가장 먼저 고친다.**
구분자 없는 사업자등록번호(10자리 연속 숫자)는 다른 숫자와 구분이 어려워 **의도적으로**
잡지 않는다. 나머지는 앞으로 넓혀 갈 대상이다.

> 이 표에서 뺀 항목: 시/도 축약형(#396), 전화 괄호 표기(#397), 여권 소문자(#398),
> 이메일 한글 로컬파트(#400). 모두 코드와 회귀 테스트로 고쳤다.

...
**측정 기준: #394/#399 규칙판 정비 병합 시점 main · 규칙 전용 모드(LLM 미사용)** — 이름 규칙판 정비(역할어·직함 어휘 확장, 역할어 뒤 조사 처리, 일반명사 정지어 단어 경계 판정)가 반영된 재측정값이다. 이름 외 10종은 #370 시점(#339/#340 표기 변형 반영)과 동일하다
...
core [#252](https://github.com/ChoHyeonChan/maskingtape/pull/252)가 고쳤다. 하드코딩 5개 부서어만
막는 방식이라 그 밖의 흔한 업무어("차량"/"허가" 등)가 [#255](https://github.com/ChoHyeonChan/maskingtape/issues/255)에서
여전히 뚫리던 잔여 위험(negative 문서 오탐 23건)은 #394/#399 규칙판 정비가 정지어를 일반명사
목록으로 넓히고 **단어 경계**로 걸러 0건으로 없앴다(precision 0.859 → 0.954). 같은 정비에서 실무
직함(총무·매니저·간호사·변호사 등, [#394](https://github.com/ChoHyeonChan/maskingtape/issues/394))과
서식 역할어(환자명·명의자·예금주·서명자 등)를 어휘에 넣고 역할어 뒤 조사("담당자는 X")를 처리해
recall도 0.668 → 0.869로 올랐다 — 남은 과제는 문맥 단서가 없는 이름과 2음절 이름+조사·직함이다:

- **이름** — 한국어 이름은 형태만으로 구분되지 않아 규칙만으로는 문맥 없는 이름을 놓친다. 이것이 **로컬 LLM 하이브리드**(`--llm`)가 필요한 이유이고, 그 효과는 [#46](https://github.com/ChoHyeonChan/maskingtape/issues/46)에서 같은 데이터셋으로 비교 측정한다 — 하이브리드로 켜면 F1 **0.923**(precision 0.920 · recall 0.927 — #339/#340 반영본 데이터셋에서 로컬 Ollama(qwen2.5:7b)로 측정). 규칙판은 그 뒤 #394/#399 정비로 0.752 → 0.910이 되어 격차가 줄었고, 하이브리드는 규칙판을 0.75 안전망으로만 쓰므로 그 정비로 흔들리지 않는다. 상세는 [bench/](bench/) 참고. ※ LLM 판단은 결정적이지 않아 재현 시 소수점 셋째 자리가 흔들릴 수 있다(규칙 전용 지표는 결정적이라 그대로 재현된다).
...
| **이름** | 로컬 LLM(`--llm`)을 켜면 F1 0.923 | 규칙 전용은 재현율 0.869 — **여덟 중 하나를 놓친다**(문맥 단서가 없는 이름, 2음절 이름+조사·직함) |
...
| 놓치는 입력 | 잡히는 입력 | 종류 |
|---|---|---|
| `세종 한누리대로 2130` | `세종특별자치시 한솔동 123` | 주소 — 세종 축약형. 세종은 시/군/구 단계가 없어 축약형 조건(뒤에 시/군/구)을 만족하지 않는다 |
| `서울강남구 테헤란로 123` | `서울 강남구 테헤란로 123` | 주소 — 축약형과 구를 붙여 씀 |
| `1234567891` | `123-45-67891` | 사업자등록번호 — 구분자 없음 |
| `우리 홍길동 1002-123-456789`, `1002-123-456789 우리 홍길동` | `우리 1002-123-456789`, `1002-123-456789 (우리)` | 계좌번호 — "우리"·"하나"·"국민"처럼 일상어와 겹치는 은행 이름은 번호 바로 앞이나 번호 뒤 괄호 안에 있을 때만 문맥으로 본다(`신한 홍길동 110-…`처럼 헷갈리지 않는 이름은 잡는다) |
| `신한 110 123 456789`, `신한 110.123.456789` | `신한 110-123-456789` | 계좌번호 — 하이픈 대신 공백·점으로 나눈 번호 |
...
> 생년월일 2자리 연도(#399), 시·군 뒤에 바로 도로명이 오는 주소(#425).
> 모두 코드와 회귀 테스트로 고쳤다.
...
**이름도 [#394](https://github.com/ChoHyeonChan/maskingtape/issues/394) 정비로 이 순환에 부분적으로 들어왔다** — 새로 넣은 서식 역할어 34개 중 11개가
생성기 템플릿이 쓰는 단어와 같아, 그 11개를 빼고 재측정하면 F1이 0.910 → 0.802, recall이
0.869 → 0.702로 내려간다. 즉 **규칙 전용 recall 0.869는 실세계 추정치가 아니라 상한**이다
...
| `apps/desktop/` | Flutter 데스크톱 앱 (드래그&드롭 배치 처리, **Windows 전용**) | [@stayalive000](https://github.com/stayalive000) |

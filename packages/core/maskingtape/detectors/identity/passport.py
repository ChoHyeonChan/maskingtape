...
1. 정규식으로 대한민국 여권번호 형식을 찾는다(외교부 차세대 전자여권 / MS Purview 확인).
   ...
   - 여권번호는 대문자로 발급되지만 문서에는 소문자로 적히는 일이 흔하다. 대소문자를
     가리지 않는다 — 대소문자 하나로 통째로 놓치면 그대로 유출이다.
...
# IGNORECASE: 소문자 표기(m12345678)도 같은 여권번호다. 발급은 대문자지만 사용자가
# 옮겨 적을 때 소문자가 섞인다 — 놓치면 유출이라 대소문자를 구분하지 않는다.

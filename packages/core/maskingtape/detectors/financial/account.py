...
2. 그래서 "은행"·"뱅크"나 계좌 관련어("계좌","입금","이체"…)가 매치 근처(±15자)에 있을 때만
   탐지한다 — 문맥이 없으면 버린다. 개별 은행 이름("국민","우리")은 일반 단어와 겹쳐 쓰지 않는다
   (아래 _ALL_CUES). (사업자등록번호 교훈: 체크섬이 없으면 문맥 + 측정.)

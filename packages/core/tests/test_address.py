...
# --- #465: 읍·면 이름 앞부분을 군·구로 읽는 부분 유출, 붙여 쓴 구, 세종 축약형 ---


def test_locality_name_containing_gun_or_gu_is_not_cut_as_gun_gu():
    """읍·면 이름 가운데 '군'·'구'가 들어 있어도 그 앞부분을 군·구로 읽으면 안 된다.

    "장군면"의 "장군", "대구면"의 "대구"를 군·구로 읽고 끝나서 "면 …"이 원문으로 남았다(부분 유출).
    시/군 앵커에서는 동/읍/면이 없다고 보고 통째로 버렸다(전체 미탐).
    """
    _assert_detected_whole([
        "세종특별자치시 장군면 대학길 12",
        "전라남도 강진군 대구면 청자촌길 12",
        "세종시 장군면 대학길 12",
        "강진군 대구면 청자촌길 12",
        "전남 강진군 대구면 청자촌길 12",
    ])
...
def test_gu_reading_is_unchanged_before_non_locality_words():
    """낱말 끝은 동/읍/면/리·도로명일 때만 따진다. 그 밖의 말이 붙은 구는 예전처럼 구까지 가린다."""
    assert [d.text for d in detect("서울특별시 강남구청 앞")] == ["서울특별시 강남구"]
    assert [d.text for d in detect("서울특별시 강남구에 산다")] == ["서울특별시 강남구"]
    # 구·군 이름 안에 동/읍이 들어 있어도(남동구·영동군·구로구) 구·군으로 읽는다.
    _assert_detected_whole([
        "인천광역시 남동구 구월동 123",
        "충청북도 영동군 영동읍 계산로 12",
        "서울 구로구 구로동 123",
...
def test_gu_glued_to_province_or_si_is_detected_whole():
    """시/도(정식명·축약형)나 시 바로 뒤에 구를 붙여 써도 끝까지 가린다.

    세 앵커 모두 시/도(또는 시) 뒤에 공백이 있어야 구를 찾아서 이런 표기를 통째로 놓쳤다.
    """
    _assert_detected_whole([
        "서울강남구 테헤란로 123",
        "부산해운대구 해운대로 264",
...
def test_glued_gu_must_end_as_a_word():
    """붙여 쓴 구는 구 이름에서 낱말이 끝날 때만 받는다. 다른 낱말의 일부는 그대로 둔다."""
    for text in (
        "서울강남구청역 3번 출구",
        "서울강남구에 산다",  # 축약형은 동/도로명까지 가야 주소로 본다(#396)
        "경북대구 지역",
        "서울특별시청 앞",
        "서울특별시민 여러분",
        "서울사람 강남구 좋아",  # 축약형에 한글이 붙은 다른 낱말
        "경기도시공사 12층",
        # 두 글자 낱말("인구")은 구로 받지 않는다. 두 글자 구는 중구·동구·서구·남구·북구뿐이다.
        "경기도인구 1400만 돌파",
        "부산인구 이동이 많다",
        "서울인구 감소로 학교가 줄었다",
    ):
        assert detect(text) == [], f"주소가 아닌 말을 잡음: {text!r}"


def test_sejong_without_si_is_detected_whole():
    """세종은 시/군/구 단계가 없어 "세종 한누리대로 2130"처럼 줄여 쓰면 통째로 놓쳤다."""
    _assert_detected_whole([
        "세종 한누리대로 2130",
        "세종 도움6로 11",
        "세종 한솔동 123",
...
def test_sejong_without_si_requires_a_building_number():
    """'세종'은 인물·대학·도로 이름에도 흔한 말이라, 번지까지 이어질 때만 주소로 본다."""
    for text in (
        "세종대왕 동상 앞에서 만나요.",
        "세종 대왕의 업적",
        "세종 대왕로를 걷다",
        ...
        assert detect(text) == [], f"주소가 아닌 문장을 잡음: {text!r}"


def test_building_number_followed_by_josa_is_detected():
    """번지 뒤에 조사가 붙어도 잡는다. 문장 속 주소에는 거의 항상 조사가 붙는다.

    시/군 뒤 도로명(#425)과 세종 축약형은 번지 뒤에 한글이 붙으면 거절해서("12명이" 같은 단위 명사를
    막으려고) "김포시 김포대로 123에 거주"를 통째로 놓쳤다.
    """
    cases = {
        "김포시 김포대로 123에 거주": "김포시 김포대로 123",
        "파주시 금릉로 77에서 만나요": "파주시 금릉로 77",
        "이천시 부악로 25-1까지 와 주세요": "이천시 부악로 25-1",
        "김포시 김포대로 123번지에 거주": "김포시 김포대로 123번지",
        "세종 한누리대로 2130으로 보내주세요": "세종 한누리대로 2130",
        "세종 한솔동 123에 거주": "세종 한솔동 123",
    }
    for text, expected in cases.items():
        assert [d.text for d in detect(text)] == [expected], text


def test_number_suffix_that_reads_as_a_count_is_not_a_building_number():
    """'만'·'도'는 조사이면서 수를 세는 말로도 읽혀("12만 명", "30도") 번지 뒤 조사로 받지 않는다."""
    for text in (
        "서울시 공무원들로 12만 명이 참여했다.",
        "제주시 해안도로 30도 넘는 더위",
        "세종 이동 3회",
    ):
        assert detect(text) == [], f"평범한 문장을 주소로 잡음: {text!r}"
...
def test_number_with_josa_after_road_like_word_is_over_masked():
    """알려진 한계: 도로명처럼 생긴 말 뒤의 수에 조사가 붙으면 번지와 구분되지 않는다.

    과다 마스킹이라 안전한 쪽이라 그대로 둔다. 단위 명사가 붙으면("3건이") 위 반례 테스트에서 걸러진다.
    """
    assert [d.text for d in detect("성남시 조례로 3에 따라 처리한다.")] == ["성남시 조례로 3"]


def test_a_match_does_not_swallow_the_start_of_the_next_address():
    """매칭이 뒤 주소의 시작을 삼키면 그 주소가 원문으로 남는다.

    동이 없어 버리는 매칭("부산진구 대구", "서울 강남구 대구")이나 구 자리가 "대구광역시"의 "대구"를
    먹은 매칭이 끝난 자리부터 다시 찾으면, 그 안에서 시작하는 "대구 수성구 …"를 시도하지 않았다.
    """
    cases = {
        "부산진구 대구 수성구 범어로 123": ["대구 수성구 범어로 123"],
        "관할: 부산진구\n대구 수성구 범어로 123": ["대구 수성구 범어로 123"],
        "서울 강남구 대구 수성구 범어로 123": ["대구 수성구 범어로 123"],
        "서울특별시 강남구\n대구광역시 수성구 범어동 123": ["서울특별시 강남구", "대구광역시 수성구 범어동 123"],
    }
    for text, expected in cases.items():
        assert [d.text for d in detect(text)] == expected, text


def test_address_list_is_not_chained_into_one_span():
    """줄마다 쓴 주소 목록은 줄마다 따로 잡는다. 구 자리가 다음 줄의 "대구"를 먹으면 사슬처럼 이어졌다."""
    text = "\n".join(["대구광역시 수성구"] * 3)
    assert [d.text for d in detect(text)] == ["대구광역시 수성구"] * 3


def test_merged_province_name_is_an_anchor():
    """2026-06-30 신설된 "전남광주통합특별시"도 시/도로 읽는다.

    목록에 없어서 시 이름으로 착각했고, 뒤에 시가 오면("… 나주시 …") 시/도 이름이 원문으로 남았다.
    """
    _assert_detected_whole([
        "전남광주통합특별시 나주시 다시면 가흥리 123",
        "전남광주통합특별시 광산구 첨단중앙로 123",
    ])

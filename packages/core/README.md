...
이름 외 10종은 형태와 체크섬으로 완전히 잡힌다. 이름은 형태만으로 구분되지 않아 **로컬 LLM 하이브리드**(`--llm`)로 보완하며, 켜면 이름 F1이 **0.752 → 0.923**으로 오른다(LLM 판단은 결정적이지 않아 재현 시 흔들릴 수 있다). 상세·재현 방법은 [저장소](https://github.com/ChoHyeonChan/maskingtape) 참고.

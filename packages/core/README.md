...
**주민등록번호**(체크섬 검증) · **전화번호**(휴대폰·유선·070·050X, +82 표기) · **이메일** · **주소**(행정구역·도로명) · **신용카드번호**(Luhn 검증) · **계좌번호** · **사업자등록번호** · **여권번호** · **생년월일**(문맥 앵커) · **운전면허번호**(형식·지역코드) · **이름**(규칙 + 로컬 LLM 문맥 판단)
...
|---|---|---|---|
| 이름 외 10종 (주민등록번호 · 전화번호 · 이메일 · 주소 · 카드 · 계좌 · 사업자 · 여권 · 생년월일 · 운전면허) | 1.000 | 1.000 | 1.000 |
| 이름 (규칙 전용) | 0.954 | 0.869 | 0.910 |
| **전체** | **0.983** | **0.950** | **0.966** |

이름 외 10종은 형태와 체크섬으로 완전히 잡힌다. 이름은 형태만으로 구분되지 않아 **로컬 LLM 하이브리드**(`--llm`)로 보완하며, 켜면 이름 F1이 **0.923**까지 오른다(규칙 전용 0.910; 하이브리드 수치는 로컬 Ollama 측정값이라 재현 시 흔들릴 수 있다). 상세·재현 방법은 [저장소](https://github.com/ChoHyeonChan/maskingtape) 참고.
...
- 🔒 **목적지가 로컬이어도 원문이 밖으로 나가는 길도 막는다(#468).**
  - 요청은 `HTTP_PROXY` 같은 프록시 설정을 거치지 않고 직접 연결로만 보낸다(회사망 프록시로 원문이 새지 않게). 리다이렉트도 따라가지 않는다.
  - Ollama의 클라우드 모델(이름이 `cloud`로 끝남, 요청을 ollama.com으로 넘김)은 원문을 보내기 전에 거부한다. `ollama cp`로 이름을 바꾼 별칭도 첫 요청 전에 Ollama 모델 정보(`/api/show`의 `remote_host`·`remote_model`)로 한 번 더 확인해 막는다.
  - `http://evil.com@localhost`처럼 검사한 주소와 실제 접속 주소가 달라질 수 있는 형식은 거부하고, 검증한 부품으로 주소를 다시 조립해 쓴다.

...
_TEMPLATES = [
    ...
    # 직함이 이름 뒤가 아니라 앞에 오는 형태(#239) — "대표 홍길동", "부장 김철수"도 core가
    # 이제 잡는다(#213과 대칭인 사각지대). 뒤 직함과 동일하게 3글자 이상 풀네임만 인정된다.
...
_NEGATIVE_TEMPLATES = [
    ...
    "정기 이사가 참석했습니다.",
    "홍보 사원이 채용되었습니다.",
    # 직함이 이름 앞에 오게 되면서(#239) "대표이사"가 띄어써지는 경우("대표 이사가") "이사"가
    # 성씨 "이"+"사"로 오탐되면 안 된다 — core가 "이사"를 _NON_NAME_WORDS에 추가해 막았다.
    "대표 이사가 참석했습니다.",
    "대표 이사회에서 발표했습니다.",
    # #247을 고친 core #252는 하드코딩된 5개 부서어(구매/정기/홍보/안전/노무)만 차단한다 —
    # 목록 밖의 흔한 업무어("차량"·"허가"·"성과"·"안내", 전부 성씨와 우연히 겹침)는 조사가
    # 붙으면("차량이") 여전히 오탐된다(#255). core 이슈로 남겼고, 잔여 위험을 여기서 실측한다.
    "대표 차량이 배정되었습니다.",
    "대표 허가가 필요합니다.",
    "부장 성과가 좋았습니다.",
    "팀장 안내가 시작되었습니다.",

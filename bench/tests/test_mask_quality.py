...
def test_birth_date_and_driver_license_label_fallback_uses_korean_labels():
    """#282(core 대응 완료) — birth_date가 `DEFAULT_LABELS`에 없어(#266/#271에서 누락)
    `[생년월일]`이 아니라 `[birth_date]`(kind 원문 그대로)로 새던 버그를 core#267 PR에서
    같이 고쳤다(driver_license 추가하며 같은 실수를 반복하지 않도록 둘 다 등록). 이제
    두 kind 모두 한글 라벨로 정상 폴백되는지 고정해둔다."""
    rng = random.Random(51)
    cases = [
        ("birth_date", BirthDateDetector(), "생년월일은 {}입니다."),
        ("driver_license", DriverLicenseDetector(), "운전면허 {}로 확인됨."),
    ]
    for kind, detector, template in cases:
        entity = generate_entity(kind, rng, difficulty="easy")
        text = template.format(entity.text)
        ...
        for name, result in (("label", label_result), ("pseudonym", pseudonym_result)):
            assert entity.text not in result, f"{kind}/{name} 전략에서 원본이 그대로 남음: {result!r}"
            assert f"[{DEFAULT_LABELS[kind]}]" in result, f"{kind}/{name} 라벨 폴백 형식이 아님: {result!r}"

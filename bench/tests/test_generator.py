...
def test_address_bunji_literal_chain_now_detected():
    ...
    이슈 원문 예시는 "서울"(축약형)을 썼는데, 당시 AddressDetector는 축약형 시/도명 자체를
    인식하지 못해(번지 유무와 무관하게 항상 빈 리스트) 별개의 변수가 섞여 있었다(축약형은
    이후 #396에서 지원을 추가했다). 이 테스트는 정식 명칭("서울특별시")으로 변수를 하나만
    남겨 "번지" 리터럴의 영향만 정확히 격리한다."""
    detector = AddressDetector()

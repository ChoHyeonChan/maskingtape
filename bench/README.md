...
500건 기준 실측 결과(#195 부분 주소 반영 후 재측정, 로컬 Ollama `qwen2.5:7b`):

| 방식 | precision | recall | F1 |
|---|---|---|---|
| 규칙판 | 0.973 | 0.691 | 0.808 |
| 하이브리드(LLM) | 0.979 | 0.916 | **0.947** |
...
직접 판단해 단서 없는 이름까지 잡아내면서(recall 0.691→0.916) F1도 더 높다(0.808→0.947) —
이제 격차의 대부분은 "문맥 단서가 아예 없는 이름"으로 좁혀졌다.

**#150·#160 반영 효과**: 규칙판 F1이 **0.676 → 0.808**로 크게 올랐다(존칭 삼킴 수정 +
단어 중간 성씨 오탐 수정). 하이브리드는 규칙판을 `min_confidence=0.75` 안전망으로만 쓰는데,
#150·#160이 고친 버그들은 confidence가 0.5로 낮게 나오는 케이스라 애초에 하이브리드 결과에
거의 반영된 적이 없었다 — 즉 두 수정은 **규칙 전용 모드 사용자에게 특히 의미 있는** 개선이다.

**이번(#195) 재측정 메모**: 하이브리드 F1이 0.934(#180 시점) → 0.947로, 지금까지의 재측정
중 가장 큰 폭으로 움직였다. name 탐지 로직 자체는 이 PR에서 전혀 안 건드렸으니 원인은
로직 변경이 아니라, `gen_address`가 매 문서마다 난수를 하나씩 더 소비하게 되면서 그
뒤에 이어지는 name 관련 난수 시퀀스가 통째로 밀려 **데이터셋에 실제로 담기는 이름 예시
구성 자체가 바뀐 것**이다(#180 재측정 때도 같은 이유로 규칙판 수치가 소폭 흔들렸던 것과
동일한 메커니즘 — 다만 이번엔 흔들림 폭이 더 컸다). 재현 시드(42)는 고정돼 있어 같은
코드로는 항상 같은 결과가 나오므로 측정 자체는 유효하다.

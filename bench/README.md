...
2026-09-05)**: `apps/api`를 띄우고 다양한 실무 문서 문장을 `/scan`으로 직접 보내보니, 규칙판의
문맥 단서 어휘(`_TITLE_CUES`/`_PREFIX_CUES`/`_SUFFIX_CUES`, name.py)가 17개 직함 + 10개
역할어뿐이라 실무에서 흔한 다른 직함("총무"·"매니저"·"상무"·"전무"·"국장"·"지점장"·"간호사"·
"변호사"·"회계사"·"코치"·"감독"·"강사"·"인턴"·"팀원") 옆 이름은 **3글자 이상 완전한 성+이름도
예외 없이** 그대로 새어나간다(15개 표본 중 14개 미탐 — "부사장"만 "사장"의 부분 문자열이라
우연히 잡힘). #213/#239가 말하는 "2음절 이름만 설계상 놓친다"와 달리 이건 아직 다뤄지지 않은
어휘 커버리지 갭이라 core 이슈로 등록했고, bench는 현재 동작(미탐)을 회귀 테스트로
고정해뒀다(`test_common_title_words_outside_cue_vocabulary_miss_the_name`) — 어휘가 넓어지거나
하이브리드가 기본이 되면 테스트가 실패하며 알려준다. 위 표의 recall 0.668(규칙판)이 낮은 이유가
"문맥 단서가 아예 없는 이름"뿐 아니라 "흔하지만 목록에 없는 직함 옆 이름"도 상당수 차지할
가능성을 보여준다.

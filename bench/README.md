...
500건 기준 최신 실측(#246 직함-이름 순서 반영 후 재측정, seed=42):
...
| address | 1.000 | 0.988 | 0.994 | [#248](https://github.com/ChoHyeonChan/maskingtape/issues/248)에서 새로 발견 — 시/도명 뒤 계사 어미("입니다"/"예요")는 #196의 조사 화이트리스트에 없어 여전히 미탐(FN 1건). core 대응 대기, 아래 참고 |
| name | 0.937 | 0.657 | 0.772 | [#239](https://github.com/ChoHyeonChan/maskingtape/issues/239)에서 core가 지원하게 된 **직함이 이름 앞에 오는 형태**("대표 홍길동")를 새로 섞으면서 precision이 크게 떨어짐(0.984→0.937, FP 4건→16건) — 절반 이상이 [#247](https://github.com/ChoHyeonChan/maskingtape/issues/247)에서 새로 발견한 core 버그 때문. 아래 참고 |
...
"주소는 X입니다"는 자기 주소를 설명하는 매우 흔한 표현인데도 여전히 놓친다 —
[#248](https://github.com/ChoHyeonChan/maskingtape/issues/248)로 core에 새로 남겼다(코드는
안 고침, 회귀 테스트로 현재 동작 고정).
...
**그런데 이번엔 진짜 새 버그를 발견했다** — [#247](https://github.com/ChoHyeonChan/maskingtape/issues/247).
#239의 "성+2음절 이상 풀네임만 인정" 가드는 2음절 후보 바로 뒤에 조사가 공백 없이 붙으면
(정상적인 한국어 주어 표기) 뚫린다. `_NAME_RE`의 선택적 2번째 글자는 님/씨/군/양 4글자만
제외해서, 조사("이"/"가")도 그대로 삼켜 "3글자"로 둔갑시키기 때문이다:

```python
NameDetector().detect("대표 정기가 참석했습니다.")
# -> [('정기가', 0.5)]  #213/#239의 negative 예시였던 "정기"(부서어)가 조사와 함께 오탐
NameDetector().detect("구매 부장이 결재했습니다.")
# -> []  공백으로 분리된 경우(제목 단어가 뒤따름)는 정상 — 가드가 이 케이스는 막음
```

역설적으로, #213/#239가 negative로 검증한 "부서어 + 공백 + 직함"(예: "구매 부장이") 형태는
가드가 잘 막는데, **그 부서어가 직함 뒤에서 문장의 주어로 자연스럽게 쓰이면**(오히려 실제
문서에 더 흔한 표기) 가드가 뚫린다. 500건 재측정에서 새로 늘어난 name FP 16건 중 9건이 바로
이 패턴이었다(직함-앞 positive 템플릿이 난수로 2음절 이름을 뽑을 때마다 재현됨). 나머지는
기존에 이미 알려진 한계들이다 — "조사 이" 삼킴(4건, "저는 김규이고" 패턴)과 #158 잔여 패턴(3건,
"민원인"/"지원자"류). core에 #247로 남겼고(코드는 안 고침), 회귀 테스트로 현재 동작을
고정해뒀다.
...
500건 기준 실측 결과(#246 재측정) — mask 기준 유출률 12.1%(931건 중 113건, 전부 완전유출 —
`name` 112건 + **`address` 1건, 신규**). `address` 1건은 [#248](https://github.com/ChoHyeonChan/maskingtape/issues/248)에서
새로 발견한 계사 어미("입니다"/"예요") 미탐 버그가 그대로 마스킹 누락으로 이어진 것이다 —
#196으로 조사(에/로 등)는 고쳐졌지만 계사 어미는 여전히 사각지대라는 걸 마스킹 결과에서도
재확인했다. `phone`/`email`/`rrn`/`card`/`biz_reg`/`passport`/`account`는 유출 0건, 부분 유출도
...
500건 기준 실측 결과(#246 재측정): 임계값 0.5 이하에서는 precision이 0.981(오탐 16건)이다 —
전부 `name` kind, confidence 정확히 0.5. 직접 재현해 원인을 셋으로 분류했다: **9건**은
[#247](https://github.com/ChoHyeonChan/maskingtape/issues/247)(직함-앞 3글자 가드가 조사
삼킴으로 뚫림, 이번에 새로 발견 — 위 참고), **4건**은 `name.py`에 이미 문서화된 한계(조사
'이' 삼킴, 예: "저는 김규이고"에서 "김규" 대신 "김규이"를 잡음), **3건**은
[#158](https://github.com/ChoHyeonChan/maskingtape/issues/158)의 잔여 패턴("민원인"/"지원자"류,
2026-08-10 재오픈). 임계값 0.7부터 이 FP가 전부 걸러져 precision이 1.000이 되지만,
**recall은 0.865(임계값 0.5 이하) → 0.569(0.7) → 0.537(0.8) → 0.523(0.85) → 0.513(0.9) →
0.347(1.0)로 계속 떨어진다** — name의 규칙판 confidence 구간이 잘려나가는 게 대부분이다.

**🚨 임계값을 0.6 초과로 올리면 안 되는 진짜 이유**: 0.5→0.7 구간에서 recall이 0.865→0.569로
...
사라진다** — RRN(재측정 기준 threshold 0.9에서 16.7%만 걸러짐, 확률적)보다 훨씬 심각하다.
...
**address(주소)도 confidence 임계값에 취약하다**: #196이 머지돼 recall 자체는 1.000에
가깝지만(#248로 FN 1건 남음), 부분 주소(#195)가 만든 confidence 0.5(3건)/0.65(4건) 구간은
여전히 남아있다 — 정답 address 81건 기준, 임계값 0.7에서 recall이 0.914로, 0.95 이상에서는
0.741까지 떨어진다.

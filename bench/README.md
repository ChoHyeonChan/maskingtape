...
500건 기준 실측 결과(#150 이름 존칭 삼킴 버그 수정 반영 후 재측정) — mask 기준 유출률 10.9%
(1007건 중 110건, 전부 완전유출·전부 `name`). #150 이전(15.7%, 1066건 중 167건)보다 크게
낮아졌다 — 규칙판 이름 recall이 0.529→0.675로 오르면서 완전 유출(탐지 자체 실패) 건수가 그만큼
줄었기 때문이다. `phone`/`email`/`rrn`/`address`/`card`/`biz_reg`/`passport`는 유선전화·plus
이메일·서브도메인·복합 문장이 섞여도 유출 0건, 부분 유출도 0건 — core 탐지기들이 새로 추가된
kind·표기·문장 구조에서도 경계까지 정확히 맞춘다는 뜻이다. label/pseudonym도 같은 패턴(유출은
전부 `name`, 길이 보존율만 전략별로 다르게 정상 표시)을 유지한다 — 자세한 비교 방법론은 위 참고.
...
500건 기준 실측 결과(#150 이름 존칭 삼킴 버그 수정 반영 후 재측정): 임계값 0.5 이하에서는
precision이 0.990(오탐 9건)으로 완벽하지 않다 — #150 이전(0.982, 오탐 16건)보다 줄었다. 직접
확인해보니 9건 전부 `name` kind, confidence 정확히 0.5(문맥 단서 없이 형태만으로 판단한 최저
확신도 케이스)였다. 그중 8건은 `name.py`에 이미 문서화된 한계(성씨+이름 뒤 조사 '이'가 이름
글자와 겹쳐 함께 삼켜지는 경우, 예: "전현이고"에서 "전현" 대신 "전현이"를 잡음 — 규칙만으로는
조사와 이름 글자를 구분 못 해 LLM판(name_llm)에 맡기기로 한 케이스)였고, 나머지 1건("...감지되어
양빈도님께..."에서 "지되어"를 이름으로 오탐)은 단일 글자 존칭(`님`/`씨`/`군`/`양`)이 단어 경계
없이 매칭되면서 뒤따라오는 별개의 실제 이름("양빈도")의 첫 글자를 자기 존칭인 것처럼 오인식한
새로운 패턴이었다 — 별도 core 이슈로 제안 예정, 이 PR 범위는 아니다. 임계값 0.7부터 이 FP가
전부 걸러져 precision이 1.000이 되지만, **recall은 0.884(임계값 0.5 이하) → 0.647(0.7) →
0.452(1.0)로 계속 떨어진다** — name의 규칙판 confidence 구간이 잘려나가는 게 대부분이다. 즉
**임계값을 높여도 얻는 정밀도 이득보다 recall 손해가 훨씬 크므로**, 지금 core 기준으로는
기본값(필터 없음)을 유지하는 게 낫다는 근거가 된다.
...
500건 기준 실측 결과(로컬 Ollama `qwen2.5:7b` 기준, #150 규칙판 존칭 삼킴 버그 수정 + 외국인등록번호(#148)·
여권 distractor(#145) 데이터까지 전부 반영 후 재측정):

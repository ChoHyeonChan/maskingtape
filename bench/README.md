...
500건 기준 실측 결과(로컬 Ollama `qwen2.5:7b` 기준, #128 LLM 이름 존칭(님/씨) 스팬 정합 반영 후 재측정):

| 방식 | precision | recall | F1 |
|---|---|---|---|
| 규칙판 | 0.924 | 0.531 | 0.675 |
| 하이브리드(LLM) | 0.987 | 0.918 | **0.951** |

규칙판은 앞뒤에 역할어·존칭 같은 문맥 단서가 없으면 아예 탐지하지 않도록 설계돼 오탐은
적지만(precision 高), 그만큼 단서 없는 이름은 다 놓친다(recall 低). 하이브리드는 LLM이 문맥을
직접 판단해 단서 없는 이름까지 잡아내면서(recall 0.531→0.918) precision도 오히려 더 높다
(0.924→0.987) — 규칙판이 흔한 단어를 성씨로 오인하던 오탐 일부를 LLM이 걸러낸 결과로 보인다.

**#128 반영 효과(회귀 이전 실측 대비)**: 하이브리드 F1이 **0.933 → 0.951**로 올랐다. `허성님`처럼

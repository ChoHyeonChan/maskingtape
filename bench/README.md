...
500건 기준 최신 실측(엣지 케이스 데이터 확대 후 재측정, seed=42):

| kind | precision | recall | f1 | 비고 |
|---|---|---|---|---|
| rrn / phone / email / address / card | 1.000 | 1.000 | 1.000 | 유선전화·plus 이메일·서브도메인·복합 문장 추가돼도 그대로 유지 |
| name | 0.881 | 0.509 | 0.645 | 규칙판. 문맥 단서 없으면 탐지 안 함(오탐↓재현율↓) — 아래 "이름 탐지 방식 비교" 절 참고 |

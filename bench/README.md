...
그대로 유출됐다 — 같은 모델·데이터에서 0.75 구성은 0.944 / 0.922 / 0.933(오탐 21 · 미탐 30)이었다.
...
데이터가 섞이며 0.752로 다시 내려갔다. 예전 하이브리드는 규칙판을 `min_confidence=0.75` 안전망으로만
써서, confidence 0.5짜리 케이스에 대한 규칙판 수정은 하이브리드에 거의 반영되지 않았다. #476부터는
규칙판을 제한 없이 안전망으로 쓰므로 **규칙판 개선이 하이브리드에도 그대로 이어진다**.

...
500건 기준 실측 결과(main 병합 후 재측정) — 세 전략 모두 동일하게 나온다:

| 전략 | 유출률 | 유출 항목 수 | 길이 보존율 |
|---|---|---|---|
| mask | 15.7% | 142건(전부 완전유출) | 100.0% (정상 — 구조 보존 계약) |
| label | 15.7% | 142건(전부 완전유출) | 23.8% (정상 — `[라벨]`로 바뀌니 길이가 다름) |
| pseudonym | 15.7% | 142건(전부 완전유출) | 50.2% (정상 — 가짜 값 길이가 원본과 다름) |

유출 142건은 전부 `name`(규칙판이 문맥 단서 없어 아예 탐지 못 한 경우)이고, 전략과 무관하게
똑같다 — 유출 여부는 **탐지가 됐는지**로 결정되지, 어떤 방식으로 가리는지와는 무관하다는 걸
숫자로 확인한 것. `phone`/`email`/`rrn`/`address`/`card`는 세 전략 다 유출 0건. 부분 유출은
아직 0건인데(mask 기준), 지금 core 탐지기들이 경계까지 정확히 맞추고 있다는 뜻이다.
...
500건 기준 실측 결과(로컬 Ollama `qwen2.5:7b` 기준, main 병합 후 실제 core로 재측정):

| 방식 | precision | recall | F1 |
|---|---|---|---|
| 규칙판 | 0.894 | 0.553 | 0.683 |
| 하이브리드(LLM) | 0.965 | 0.934 | **0.949** |

규칙판은 앞뒤에 역할어·존칭 같은 문맥 단서가 없으면 아예 탐지하지 않도록 설계돼 오탐은
적지만(precision 高), 그만큼 단서 없는 이름은 다 놓친다(recall 低). 하이브리드는 LLM이 문맥을
직접 판단해 단서 없는 이름까지 잡아내면서(recall 0.553→0.934) precision도 오히려 더 높다
(0.894→0.965) — 규칙판이 흔한 단어를 성씨로 오인하던 오탐 일부를 LLM이 걸러낸 결과로 보인다.

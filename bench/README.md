...
>
> 1. **F1 1.000은 "실세계 recall 상한"이 아니라 "생성기↔탐지기 경계정확성" 지표다.** 이 벤치의
>    생성기(`bench/generator/entities.py`)는 core 탐지기의 정규식·체크섬 로직을 **직접
...
`birth_date`(생년월일)는 core `BirthDateDetector`가 새로 추가되면서([#266](https://github.com/ChoHyeonChan/maskingtape/issues/266)/[#271](https://github.com/ChoHyeonChan/maskingtape/pull/271))
생긴 10번째 kind다(#275). `account`(#180)와 같은 **문맥 앵커 하드 게이트** 설계다 —
"생년월일"/"생일"/"출생일" 같은 앵커가 날짜 바로 앞에 있어야만 잡히고, 앵커 없는 일반
날짜(계약일·근무 개시일 등)는 애초에 candidate조차 아니다. `gen_birth_date`로 "1999년
7월 21일" 류(한국어)와 "1999-07-21" 류(구분자 -/./ 세 종류) 표기 다양성을 만들고, 모든
positive 템플릿에 앵커를 반드시 포함시켰다. distractor 쪽은 기존 `gen_date`(주문일·결제일
등에 쓰이던 일반 날짜 생성기)가 마침 core의 날짜 정규식과 정확히 같은 형식을 만들고
있어서 — 즉 core 입장에선 "생년월일이든 결제일이든 형식만 보면 똑같다" — 새 코드 없이
그대로 앵커-없음 negative 세트로 재활용했다(#180의 "체크섬 있고 없고" 패턴과 유사하게,
여기선 "앵커 있고 없고"가 유일한 차이).

**직접 잡아낸 자기 회귀**: positive 템플릿 하나("신청자 {name}의 서류가...")가 이름과
조사 "의"를 공백 없이 바로 붙이는 바람에, 2음절 이름 뒤에 조사가 통째로 이름 스팬에
삼켜지는 걸 재측정 중 직접 재현으로 발견했다(`정수의`처럼 잡혀 정답 `정수`와 어긋남) —
[#247](https://github.com/ChoHyeonChan/maskingtape/issues/247)/[#255](https://github.com/ChoHyeonChan/maskingtape/issues/255)에서
이미 문서화된 "조사가 이름 글자와 겹치면 span이 밀린다"는 한계의 또 다른 사례다. 존칭
"님"을 사이에 끼워("신청자 {name}님의") 조사가 이름에 안 붙게 고쳤다 — #180 세션의 "정산
...
`driver_license`는 유출 0건, 부분 유출도 0건 — core 탐지기들이 새로 지원하게 된 표기에서도 경계까지 정확히
맞춘다는 뜻이다(체크섬 없는 RRN도 confidence 0.85로, 문맥어 있는 계좌번호도 confidence
0.6으로, 앵커 있는 생년월일도 confidence 0.9로 여전히 탐지되므로 여기엔 안 걸린다 — 아래
confidence 절 참고). label/pseudonym도 같은 패턴(유출은 전부 `name`, 길이 보존율만
전략별로 다르게 정상 표시)을 유지한다 — 자세한 비교 방법론은 위 참고.
...
**`passport`(여권번호)는 위 kind들과 다른 패턴이다 — 문맥어("여권")로 confidence를 0.9/0.6
두 값으로 가르는 유일한 kind다(체크섬이 없어서다, 위 생성 다양성 절 참고). 정답 33건 기준
실측(synth_v1.jsonl, [#392](https://github.com/ChoHyeonChan/maskingtape/issues/392)): confidence
0.9(문맥어 있음)가 26건(78.8%), confidence 0.6(문맥어 없음)이 7건(21.2%)이다. 즉 **임계값을
0.7 이상으로만 올려도 문맥어 없이 등장하는 여권번호(약 21%)가 조용히 사라진다** — account/
birth_date/driver_license처럼 100% 확정적 위험은 아니지만, RRN(13.2%)보다도 큰 비율이 한 번에
날아가는 조건부 위험이다. 두 confidence 값 모두 core 로직에 고정돼 있음을 회귀 테스트로
잠갔다(`test_generated_passport_confidence_splits_by_context_word`).

RRN(확률적 13.2%)·계좌번호·생년월일·운전면허번호(셋 다 확정적 100%)·여권번호(조건부 21.2%)에
이어 confidence 필터를 쓰면 안 되는 근거가 하나 더 늘었다.
...
|---|---|---|---|
| 규칙판 | 0.954 | 0.869 | 0.910 |
| 하이브리드(LLM) | 0.920 | 0.927 | **0.923** |

※ 규칙판 행은 #394/#399 정비 후 재측정값(정비 전 0.859 / 0.668 / 0.752). 하이브리드 행은 정비 전
측정값이다 — 하이브리드는 규칙판을 `min_confidence=0.75` 안전망으로만 쓰므로 정비의 영향이 작고,
LLM 판단은 결정적이지 않아 재측정마다 소수점 셋째 자리가 흔들린다.
...
직접 판단해 단서 없는 이름까지 잡아내면서(정비 전 규칙판 대비 recall 0.668→0.927) recall이 크게 오르고,
precision도 정비 전 규칙판보다 훨씬 높았다(0.859→0.920, 오탐 42건→31건; 정비 후 규칙판은 0.954·16건) — #255에서 정량화한 부서어
...
규칙판의 문맥 단서 어휘(`_TITLE_CUES`/`_PREFIX_CUES`/`_SUFFIX_CUES`, name.py)가 17개 직함 + 10개
역할어뿐이라 실무에서 흔한 다른 직함("총무"·"매니저"·"상무"·"전무"·"국장"·"지점장"·"간호사"·
"변호사"·"회계사"·"코치"·"감독"·"강사"·"인턴"·"팀원") 옆 이름은 **3글자 이상 완전한 성+이름도
예외 없이** 그대로 새어나갔다(15개 표본 중 14개 미탐 — "부사장"만 "사장"의 부분 문자열이라
우연히 잡힘). core가 어휘를 넓혀(직함 17 → 52개, 역할어 10 → 44개) 이제 풀네임은 잡히며, bench의
...
**LLM은 core의 부서어 오탐 가드를 모른다 — 대신 자기만의 오탐 패턴이 있다**(#223 세션에서
처음 확인, 이번에도 재확인): `LLMNameDetector`를 #255의 새 negative 템플릿에 개별로 돌려보면:

```python
LLMNameDetector(host="http://127.0.0.1:11434").detect("대표 차량이 배정되었습니다.")  # -> []  정상
LLMNameDetector(host="http://127.0.0.1:11434").detect("대표 허가가 필요합니다.")      # -> []  정상
LLMNameDetector(host="http://127.0.0.1:11434").detect("부장 성과가 좋았습니다.")      # -> [('성', 0.9)]  오탐
LLMNameDetector(host="http://127.0.0.1:11434").detect("팀장 안내가 시작되었습니다.")   # -> [('안내', 0.9)]  오탐
```

규칙판은 "차량"/"허가"를 놓치고 LLM은 그 둘을 정상 통과시키는 대신 "성"/"안내"에서 오탐
낸다 — 규칙판(#255)과 하이브리드(LLM)의 오탐 대상이 서로 다르다는 뜻이다. 규칙판의
"성+2음절 이상 풀네임만 인정" 가드(#213/#239)는 정규식으로 구현된 것이라 LLM에는 전혀
적용되지 않는다 — LLM은 문장을 보고 스스로 판단하므로, 성씨로 시작하고 문맥상 그럴듯해 보이는
단어는 개별 모델 판단에 따라 이름으로 오인식할 수 있다. 이건 bench 데이터의 문제가 아니라
**LLM 판단의 근본적인 비결정성**이므로 core 코드로 고칠 수 있는 성격이 아니다.

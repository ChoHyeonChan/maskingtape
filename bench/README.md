...
500건 기준 실측 결과(#180 계좌번호 신규 kind 반영 후 재측정, 규칙판):

| 방식 | precision | recall | F1 |
|---|---|---|---|
| 규칙판 | 0.981 | 0.715 | 0.827 |
| 하이브리드(LLM) | `<재측정 필요>` | `<재측정 필요>` | `<재측정 필요>` |

**⚠️ 하이브리드 수치는 이번 PR에서 재측정 못 함**: 로컬 Ollama 호출이 "localhost"를 IPv6(`::1`)로
먼저 시도하는데 Ollama는 IPv4(`127.0.0.1`)에만 리스닝하고 있어 연결이 `SYN_SENT`에서 멈추는
환경 문제를 겪었다(`127.0.0.1`로 명시하면 우회 가능함을 직접 재현해 확인). 우회해서 재시도했지만
500건 전체가 끝나기 전에 시간 제약으로 중단했다 — 로직 변경은 없으니(#150·#160 이후 규칙판
0.803→0.827로 자연 변동한 정도) 하이브리드도 F1 0.937 근방일 가능성이 높지만, **확정값이
아니므로 다음 세션에서 반드시 재측정 후 이 표를 갱신할 것.**

규칙판은 앞뒤에 역할어·존칭 같은 문맥 단서가 없으면 아예 탐지하지 않도록 설계돼 오탐은
적지만(precision 高), 그만큼 단서 없는 이름은 다 놓친다(recall 低) — 하이브리드는 LLM이 문맥을
직접 판단해 단서 없는 이름까지 잡아내 격차를 좁힌다는 게 기존 실측(#159/#160 기준 F1 0.937)의
결론이었다.

**#150·#160 반영 효과**: 규칙판 F1이 **0.676 → 0.827**로 크게 올랐다(존칭 삼킴 수정 +
단어 중간 성씨 오탐 수정). 하이브리드는 규칙판을 `min_confidence=0.75` 안전망으로만 쓰는데,
#150·#160이 고친 버그들은 confidence가 0.5로 낮게 나오는 케이스라 애초에 하이브리드 결과에
거의 반영된 적이 없었다(#159/#160 재측정 당시 하이브리드 F1 0.933→0.937, 오차 범위) — 즉
두 수정은 **규칙 전용 모드 사용자에게 특히 의미 있는** 개선이다.
